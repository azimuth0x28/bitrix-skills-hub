...
```
skills/<name>/
├── SKILL.md      # роутер: name, version, description, триггеры, ссылки на правила
...
Каждый скилл несёт семантическую версию в `metadata: {type, version}`; папка переносит её в проекты-потребители, поэтому любое изменение поднимает версию.

| Bump | Когда |
| --- | --- |
| patch | опечатки, форматирование, формулировки без смены смысла |
| minor | изменения контента правил, новый/изменённый файл rules, изменения описания (вкл. оптимизацию), новые примеры |
| major | смена baseline, пересмотр рекомендации, депрекация |

CI проверяет поднятие версии в каждом PR (`check-versions.py`, `check-budgets.sh`); `plugin.json` отслеживает последний релизный тег — catalog smoke падает при расхождении.
